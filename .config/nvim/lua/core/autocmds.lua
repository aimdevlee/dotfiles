...
-- Constants
local LARGE_FILE_SIZE_BYTES = 1024 * 1024 -- 1MB
local TREESITTER_MAX_FILESIZE = 100 * 1024 -- 100KB
...
vim.api.nvim_create_autocmd('BufReadPre', {
  ...
  callback = function(args)
    ...
    -- getfsize returns -1 on error, -2 for directory
    if file_size < 0 then
      return
    end
    if file_size > LARGE_FILE_SIZE_BYTES then
